...
export async function renderDashboard(container) {
  ...
  // Filter yang belum selesai (exclude approved, cancelled & printed) dan URUTKAN terbaru di atas
  const employees = allEmployees
    .filter(e => e.status !== 'approved' && e.status !== 'cancelled' && e.status !== 'printed')
...
function attachRowListeners(tbody) {
  tbody.querySelectorAll('.emp-row').forEach(row => {
    row.addEventListener('click', () => {
      ...
      if (status === 'ready_review' || status === 'approved' || status === 'cancelled' || status === 'printed') {
